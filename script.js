...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (joySlider) {
        ...
        joySlider.addEventListener('input', (e) => {
            ...
            if (val < 25) {
                joyEmoji.innerText = "😏";
                joyText.innerText = "لسه معملتيش حاجة..";
                joyText.classList.remove('max-joy');
                hasExploded = false;
            } else if (val < 50) {
                joyEmoji.innerText = "🥰";
                joyText.innerText = "شوية صغيرة";
                joyText.classList.remove('max-joy');
                hasExploded = false;
            } else if (val < 75) {
                joyEmoji.innerText = "😍";
                joyText.innerText = "مبسوطة جداً";
                joyText.classList.remove('max-joy');
                hasExploded = false;
            } else if (val < 100) {
                joyEmoji.innerText = "💖";
                joyText.innerText = "أحلى عيد في حياتي!";
                joyText.classList.remove('max-joy');
                hasExploded = false;
            } else if (val === 100) {
                joyEmoji.innerText = "🎆❤️🔥";
                joyText.innerText = "طايرة من الفرحة!! 🚀";
...
window.openLightbox = function(cardElement) {
    ...
    lightboxCaption.innerText = realImg.alt || "صورتنا الحلوة";
